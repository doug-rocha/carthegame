...
import com.nonacept.carthegame.Entidades.*;
import com.nonacept.carthegame.Game;
import com.nonacept.carthegame.IO.Levels.*;
...
public class LevelParser {
    ...
    public void loadLevel(int levelNumber) {
        ...
        String name = "";
        for (int i = 0; i < jogo.listaLevels.length && !carregar; i++) {
            if (jogo.listaLevels[i].ID == levelNumber) {
                name = jogo.listaLevels[i].Location;
                carregar = true;
            }
        }
        if (carregar) {
            ...
            System.out.println(String.format("ERRO | NÃO FOI POSSÍVEL ENCONTRAR O LEVEL %d", levelNumber));
            if (Game.DEBUGMODE) {
                Game.TA = new TextAttributes(Color.CYAN);
                Game.DEBUGCONSOLE.setTextAttributes(Game.TA);
            }
            System.out.println("NCPT | VOLTANDO AO MENU");
        }
    }
    ...
    public void loadLevel(String levelFileName) {
        ...
        try {
            ...
            xmlStream.alias("CTGMap", CTGMap.class);
            ...
            xmlStream.alias("Chegada", com.nonacept.carthegame.IO.Levels.Chegada.class);
            xmlStream.alias("ChegadaArea", ChegadaArea.class);
            xmlStream.alias("Caixa", com.nonacept.carthegame.IO.Levels.CaixoteLvl.class);
            ...
            xmlStream.alias("MapLink", com.nonacept.carthegame.IO.Levels.CenarioChanger.class);

            mp = (CTGMap) xmlStream.fromXML(arqLvl);
            ...
            jogo.chegadas = jogo.linhasChega.toArray(new com.nonacept.carthegame.Entidades.Chegada[jogo.linhasChega.size()]);
            ...
            jogo.mapLink = jogo.mapLinks.toArray(new com.nonacept.carthegame.Entidades.CenarioChanger[jogo.mapLinks.size()]);
    ...
    private void procChegadas1() {
        if (mp.chegadas != null) {
            for (int i = 0; i < mp.chegadas.length; i++) {
                com.nonacept.carthegame.Entidades.Chegada cheg = new com.nonacept.carthegame.Entidades.Chegada(0, 0);
        ...
        if (mp.areaChegadas != null) {
            for (int i = 0; i < mp.areaChegadas.length; i++) {
                com.nonacept.carthegame.IO.Levels.Chegada[] chegs = mp.areaChegadas[i].getChegadas();
    ...
    private void procCenarios1() {
        if (mp.mapChanger != null) {
            for (int i = 0; i < mp.mapChanger.length; i++) {
                com.nonacept.carthegame.Entidades.CenarioChanger link;
                mp.mapChanger[i].preProc();
                link = mp.mapChanger[i];
                jogo.mapLinks.add(link);
            }
        }
    }
}
