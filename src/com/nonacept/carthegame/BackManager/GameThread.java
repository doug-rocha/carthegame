 ...
 */
package com.nonacept.carthegame.BackManager;
...
public class GameThread implements Runnable {
    ...
    private void game() {
        ...
        do {
            ...
            for (int i = 0; i < jogo.mapLink.length; i++) {
                bgb.setColor(Color.WHITE);
                bgb.fillRect(jogo.mapLink[i].x, jogo.mapLink[i].y, jogo.mapLink[i].tamX, jogo.mapLink[i].tamY);
            }
    ...
    private void drawHud() {
        ...
        Color fundoHud = new Color(20, 20, 25), corRED = new Color(255, 25, 25), corGREEN = new Color(25, 255, 25);
        jogo.bbg.setColor(fundoHud);
        ...
        if (jogo.tempodano > 0) {
            ...
            for (int i = 1; i <= 5; i++) {
                jogo.bbg.setColor(new Color(coloracao.getRed(), coloracao.getGreen(), coloracao.getBlue(), 255 / i));
                jogo.bbg.drawRoundRect(((jogo.janelaW / 2) - (221 + i)), ((jogo.janelaH) - (24 + i)), ((201) + (i * 2)), ((16) + (i * 2)), 5, 3);
            }
            jogo.tempodano -= 6;
        }
        if (jogo.tempovida > 0) {
        ...
        if (jogo.gameTipo == 1) {
            jogo.bbg.drawString("X: " + Math.round(jogo.carro.x / Game.MODRESOL) + " Y: " + Math.round(jogo.carro.y / Game.MODRESOL), SIZES._6, ((jogo.janelaH) - 10));
        } else if (jogo.gameTipo == 2) {
            jogo.bbg.drawString("X: " + Math.round(jogo.mapX / Game.MODRESOL) + " Y: " + Math.round(jogo.mapY / Game.MODRESOL), SIZES._6, ((jogo.janelaH) - 10));
        }
        jogo.bbg.setColor(new Color(R, G, B));
        jogo.bbg.fillRoundRect(((jogo.janelaW / 2) - 220), ((jogo.janelaH) - 23), 200 - (jogo.exibindovida * 2), 15, 1, 1); //BARRA DE VIDA
        jogo.bbg.setColor(new Color(0, 100, 255));
        jogo.bbg.fillRoundRect((jogo.janelaW / 2) + 220 - (jogo.nitroexibido * 2), jogo.janelaH - 23, jogo.nitroexibido * 2, 15, 1, 1); //BARRA DE NITRO
        int cinzamento = (int) (1.65 * (100 - jogo.nitroexibido));
        jogo.bbg.setColor(new Color(90, 90, 255 - cinzamento));
        jogo.bbg.drawRoundRect((jogo.janelaW / 2) + 220 - 201, jogo.janelaH - 24, 200, 16, 5, 5);//CONTORNO BARRA DE NITRO
        jogo.bbg.setColor(new Color(R / 2, G / 2, 50));
        jogo.bbg.drawRoundRect(((jogo.janelaW / 2) - 221), ((jogo.janelaH) - 24), 201, 16, 5, 5);//CONTORNO BARRA DE VIDA
        jogo.bbg.setColor(jogo.fonte);
        jogo.bbg.drawString(jogo.iMsg.inimigos.texto + jogo.quantidade, (jogo.janelaW - (jogo.bbg.getFontMetrics().stringWidth(jogo.iMsg.inimigos.texto + jogo.quantidade)) - SIZES._6), ((jogo.janelaH) - 10));
        jogo.bbg.drawString(jogo.iMsg.nivel.texto + jogo.nivel, SIZES._120, ((jogo.janelaH) - 10));
        //INFORMAÇÕES MÚSICAS
        if (jogo.exibindoinfom) {
            ...
            jogo.bbg.drawString(jogo.iMsg.trilhaSom.texto, (jogo.janelaW - 167 + (300 - jogo.jseg1)), 96);
            jogo.jtimeInfo++;
        }
