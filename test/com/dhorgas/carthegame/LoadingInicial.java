...
import com.dhorgas.carthegame.IO.ResolParserXML;
...
import com.dhorgas.carthegame.IO.LevelParser;
...
public class LoadingInicial implements Runnable {
    ...
    void LoadSaveGame() {
        ...
        try {
            ...
            if (jogo.OSystem.contains("linux")) {
                ...
                arqSave = new File(System.getProperty("user.home") + "/Documents/CTG/saves/" + saveName + ".xsav");
            }
            if (Game.DEBUGMODE) {
    ...
    void loadGame(boolean exComp) {
        ...
        try {
            jogo.musicasd = MusicaDiretorio.listFiles();
            jogo.countmus = jogo.musicasd.length;
            ...
            for (int i = 0; i < jogo.countmus; i++) {
                ...
                if (Game.DEBUGMODE) {
                    ...
                    System.out.println(String.format("MISC | MUSICA: %s", jogo.musicasd[i].getName()));
                }
            }
    ...
    void load() {
        if (mostra) {
            ...
            DhorgasAnimIc = new ImageIcon[11];
            for (int i = 0; i < 11; i++) {
                DhorgasAnimIc[i] = new ImageIcon("arquivos/Images/DS/" + i + ".cpn");
            }
            DhorgasAnim = new Image[11];
            for (int j = 0; j < 11; j++) {
                DhorgasAnim[j] = DhorgasAnimIc[j].getImage();
            }
            ...
            try {
                ...
                if (jogo.OSystem.contains("linux")) {
                    ...
                    arqinfo = new File(System.getProperty("user.home") + "/Documents/CTG/info.ctgi");
                }
                if (Game.DEBUGMODE) {
                    Game.TA = new TextAttributes(Color.GREEN);
                    Game.DEBUGCONSOLE.setTextAttributes(Game.TA);
                ...
                if (!arqinfo.exists()) {
                    ...
                    if (jogo.OSystem.contains("linux")) {
                        ...
                        pasta = new File(System.getProperty("user.home") + "/Documents/CTG");
                    }
                    pasta.mkdir();
                    if (Game.DEBUGMODE) {
                        Game.TA = new TextAttributes(Color.YELLOW);
                        Game.DEBUGCONSOLE.setTextAttributes(Game.TA);
                        System.out.println(String.format("ALRT | ARQUIVO %s NÃO EXISTE", arqinfo.getName()));
                        Game.TA = new TextAttributes(Color.GREEN);
                        Game.DEBUGCONSOLE.setTextAttributes(Game.TA);
                    ...
                    String escrita = "arquivos/version.ctgv;arquivos/Sons/musicas;arquivos/Sons;960;720;arquivos/Sons/Inf;false;arquivos/Levels;90;0;true;";
                    pw.append(escrita);
                    escritor.close();
                    pw.close();
                    if (Game.DEBUGMODE) {
